...
class FramePackWorker:
    ...
    def _validate_runtime_tensor_size(self, tensor: torch.Tensor, operation_name: str, safety_factor: float = 256.0):
        """Validate tensor size at runtime before VAE operations with conservative safety factor"""
        tensor_numel = tensor.numel()
        max_int32 = 2**31 - 1
        
        # Apply conservative safety factor to account for VAE internal operations and upsampling
        # Reduced from 512x to 256x to allow longer videos while maintaining safety
        ...
        print(f"   PyTorch int32 limit: {max_int32:,}")
        print(f"   Validation result: {'PASS' if tensor_numel <= safe_limit else 'FAIL'}")
        
        if tensor_numel > safe_limit:
            print(f"⚠️ Tensor validation failed - will use fallback processing")
            raise RuntimeError(
                ...
                f"Will use single-frame processing instead."
            )
    ...
    def _force_single_frame_vae_decode(self, latents: torch.Tensor, vae, operation_name: str):
        """Force single-frame VAE decoding with ultra-extreme safety validation"""
        ...
        if latents.shape[2] == 1:
            ...
            try:
                ...
                print(f"⚠️ Single frame too large even with 1024x safety factor: {e}")
                return self._emergency_spatial_downsample_decode(latents, vae, operation_name)
        
        # Process frame by frame with ultra-extreme validation
        pixel_frames = []
        for frame_idx in range(latents.shape[2]):
            ...
            try:
                ...
                print(f"⚠️ Frame {frame_idx+1} too large even with 1024x safety factor: {e}")
                # Emergency spatial downsampling for this frame
                frame_pixel = self._emergency_spatial_downsample_decode(frame_latent, vae, f"{operation_name} frame {frame_idx+1}")
    ...
    def _emergency_spatial_downsample_decode(self, latents: torch.Tensor, vae, operation_name: str):
        """Emergency spatial downsampling when even single frames are too large"""
        print(f"🚨 Emergency spatial downsampling for {operation_name}")
        
        # Get original dimensions
        B, C, T, H, W = latents.shape
        
        # Try progressively smaller spatial dimensions
        downsample_factors = [2, 4, 8]
        
        for factor in downsample_factors:
            try:
                # Downsample spatially
                downsampled_latents = torch.nn.functional.interpolate(
                    latents.view(B, C*T, H, W),
                    size=(H//factor, W//factor),
                    mode='bilinear',
                    align_corners=False
                ).view(B, C, T, H//factor, W//factor)
                
                # Validate downsampled tensor
                ...
                # Decode downsampled latents
                torch.cuda.empty_cache()
                downsampled_pixels = vae_decode(downsampled_latents, vae).cpu()
                
                # Upsample back to original resolution
                final_pixels = torch.nn.functional.interpolate(
                    downsampled_pixels.view(B, 3*T, H//factor, W//factor),
                    size=(H*8, W*8),  # VAE upsamples by 8x
                    mode='bilinear',
                    align_corners=False
                ).view(B, 3, T, H*8, W*8)
                
                print(f"✅ Emergency spatial downsampling successful with {factor}x factor")
                return final_pixels
                
            except RuntimeError as e:
                print(f"⚠️ {factor}x downsampling failed: {e}")
                continue
        
        # If all downsampling attempts fail, return a placeholder
        print(f"❌ All emergency downsampling attempts failed, returning placeholder")
        placeholder_pixels = torch.zeros(B, 3, T, H*8, W*8, dtype=torch.float32)
        return placeholder_pixels
    ...
    def process_job(self, job_id: str):
        ...
        try:
            ...
            print(f"   Using F1 model: {use_f1_model}")
            ...
            if use_f1_model:
                ...
                print(f"📹 Using F1 model generation with {total_latent_sections} sections")
                for section_index in range(total_latent_sections):
                    print(f"🔄 Processing F1 section {section_index + 1}/{total_latent_sections}")
                    if not self.high_vram:
                        unload_complete_models()
                ...
                print(f"📹 Using standard model generation with {total_latent_sections} sections")
                ...
                for section_idx, latent_padding in enumerate(latent_paddings):
                    print(f"🔄 Processing standard section {section_idx + 1}/{len(latent_paddings)} (padding: {latent_padding})")
    ...
    def _decode_section(self, job_id, history_latents, history_pixels, total_generated_latent_frames,
        ...
        if not tensor_is_safe:
            ...
            print(f"⚠️ Tensor validation failed, using single-frame processing")
            print(f"   Real history latents shape: {real_history_latents.shape}")
            ...
            try:
                ...
                print(f"✅ Single-frame processing completed. Result shape: {history_pixels.shape}")
            except RuntimeError as e:
            ...
            print(f"💾 Saving intermediate video from single-frame processing: {output_filename}")
            print(f"   Video frames: {history_pixels.shape[2]}")
            print(f"   Video duration: {history_pixels.shape[2] / 30:.2f} seconds")
            save_bcthw_as_mp4(history_pixels, output_filename, fps=30, crf=mp4_crf)
            ...
            if history_pixels is None:
                ...
                if real_history_latents.shape[2] > 8:  # If more than 8 frames, use chunking
                    ...
                    for i in range(0, real_history_latents.shape[2], chunk_size):
                        ...
                        try:
                            self._validate_runtime_tensor_size(chunk_latents, f"chunk_{i//chunk_size} VAE decode", safety_factor=256.0)
                            chunk_is_safe = True
                        except RuntimeError as e:
                ...
                try:
                    self._validate_runtime_tensor_size(section_latents, "section_latents VAE decode", safety_factor=256.0)
